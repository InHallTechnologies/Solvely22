    ...
    render() {
        ...
        return( 
            <div className='header' ref = {this.props.ref}>
                {this.state.sidebarVisiblity? null: <img className='menu' src = {menu} alt= 'menu' onClick={this.toggleSidebar}/>}
                
                {this.state.sidebarVisiblity ? <Sidebar allRef = {this.props.allRef}  menuItems={this.menuItems} clickHandler={this.toggleSidebar} optionClicked = {this.toggleSidebar}/> :  null}
                
                <img className='logo' src = {icon} alt= 'solvely.22 logo' ref = {this.myRef}/>
    
                <div className='navigation-container'>
                    {
                       this.menuItems.map((item, index) => <NavigationOption handleClick={this.handleClick} key={this.menuItems[index]} styles={item === 'WORK WITH US'? 'navigation-option work-with-us': 'navigation-option' } label = {item}/>)
                    }
                </div>
            </div>
        );
